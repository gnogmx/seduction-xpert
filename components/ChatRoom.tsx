...
type ApiHistoryItem = { role: 'user' | 'assistant' | 'system'; content: string };
    ...
    setMessages([
      {
        ...
        text:
          language === 'pt'
            ? 'Olá. Como posso ajudar você hoje? Você pode me enviar uma foto do ambiente ou da pessoa para eu sugerir a melhor abordagem.'
            : language === 'en'
            ? 'Hello. How can I help you today? You can send me a photo of the environment or person so I can suggest the best approach.'
            : 'Hola. ¿Cómo puedo ayudarte hoy? Puedes enviarme una foto del ambiente o de la persona para sugerir la mejor acción.',
        timestamp: new Date(),
      },
    ]);
    setHistory([]);
  ...
  const handleSend = async () => {
    ...
    try {
      const prompt =
        currentInput ||
        (language === 'pt'
          ? 'Analise esta imagem e me dê dicas.'
          : language === 'es'
          ? 'Analiza esta imagen y dame consejos.'
          : 'Analyze this image and give me tips.');

      const response = await GeminiService.generateMultimodalResponse(language, history, prompt, currentImage || undefined);
      ...
      // ✅ IMPORTANT: history agora é formato OpenAI (role/content), sem "model" e sem "parts"
      setHistory((prev) => [
        ...prev,
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              {m.image && <img src={m.image} alt="Sent" className="max-w-full rounded-lg border border-white/10 shadow-lg" />}
