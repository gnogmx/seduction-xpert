...
import { TRANSLATIONS, STRIPE_LINKS, INITIAL_EBOOKS } from '../constants';
...
const EBooks: React.FC<{language: Language}> = ({ language }) => {
  ...
  useEffect(() => {
    const fetchEbooks = async () => {
      try {
        const data = await DataService.getProducts('ebooks');
          ...
          {ebooks.map(eb => (
            <div key={eb.id} className="group bg-[#080808] border border-zinc-800 rounded-2xl overflow-hidden hover:border-gold/50 transition-all flex flex-col">
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col">
                <h3 className="text-xl font-bold group-hover:text-gold transition-colors">{eb.title[language] || eb.title}</h3>
                <p className="text-zinc-500 text-sm line-clamp-2">{eb.description ? eb.description[language] : 'Conteúdo exclusivo SX.'}</p>
                <div className="flex items-center justify-between pt-4 mt-auto">
                  <span className="text-gold font-bold">{eb.price}</span>
                  <a 
                    href={eb.stripe_url || STRIPE_LINKS.PRODUCTS.PRICE_9_90}
                    target="_blank"
